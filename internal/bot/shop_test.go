...
func TestShopFetcher_NormalizesLevelUUIDToParentSkinUUID(t *testing.T) {
	const (
		puuid      = "puuid-shop-level"
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.Contains(r.URL.Path, "/store/v3/storefront/"):
			_ = json.NewEncoder(w).Encode(map[string]any{
				"SkinsPanelLayout": map[string]any{
					"SingleItemStoreOffers": []map[string]any{
						{"OfferID": levelUUID, "Cost": map[string]int{vp: 1775}},
					},
				},
			})
		default:
				...
				"data": []map[string]any{
					{
						...
						"displayIcon":     "https://example.com/v.png",
	...
	o := shops[0].Offers[0]
	if o.SkinUUID != parentUUID {
		t.Fatalf("SkinUUID = %q, want parent %q (storefront OfferID is level UUID)", o.SkinUUID, parentUUID)
	}
	if o.DisplayName != "Prime Vandal" || o.IconURL != "https://example.com/level.png" {
		t.Fatalf("display fields %+v", o)
	}
}
